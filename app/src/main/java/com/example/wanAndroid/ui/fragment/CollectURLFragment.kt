 ...
 * desc: 收藏网址Fragment
 */
class CollectURLFragment : Fragment() {
    ...
    /** 适配器 */
    private val adapter: CollectURLAdapter by lazy { CollectURLAdapter() }
    ...
    override fun onResume() {
        ...
        if (first) {
            ...
            rv.adapter = adapter
            //设置RecycleView的分隔线
            rv.addItemDecoration(RecyclerViewItemDecoration(requireContext()))
            //刷新数据
            page.onRefresh {
                scope {
                    //获取体系数据
                    val data = Get<ApiResponse<ArrayList<CollectURLResponse>>>(NetApi.CollectURLListAPI).await()
