...
class IntegralActivity : BaseActivity() {
    ...
    /** 数据集 */
    private lateinit var mData: ApiResponse<ApiPagerResponse<ArrayList<IntegralListResponse>>>
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        page.onRefresh {
            scope {
                //获取积分列表数据
                mData = Get<ApiResponse<ApiPagerResponse<ArrayList<IntegralListResponse>>>>("${NetApi.IntegralListAPI}/$index/json").await()
                if (first && mData.data.datas.isEmpty()) {
                    //如果第一次切换且数据为空显示空缺省页
                    showEmpty()
                } else {
                    //设置初次创建页面为否
                    ...
                    index += if (index == 1) { //下拉刷新
                        //设置数据
                        adapter.setList(mData.data.datas)
                        //翻页
                        1
                    } else { //上拉加载更多
                        if (mData.data.datas.isNullOrEmpty()) {
                            //没有更多数据，结束动画，显示内容(没有更多数据)
                            showContent(false)
                            return@scope
                        }
                        //添加数据
                        adapter.addData(mData.data.datas)
                        //翻页
                        1
                    }
                    showContent(true)
