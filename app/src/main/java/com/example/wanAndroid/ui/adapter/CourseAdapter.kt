    ...
    override fun convert(holder: BaseViewHolder, item: CourseResponse) {
        ...
        holder.getView<ImageView>(R.id.item_course_image).run {
            if (item.cover.isNotEmpty()) {
                Glide.with(context).load(item.cover).placeholder(R.drawable.bg_project).transition(DrawableTransitionOptions.withCrossFade(500))
                    .into(this)
            } else {
                Glide.with(context).load(R.drawable.bg_project).into(this)
            }
        }
        //课程标题
