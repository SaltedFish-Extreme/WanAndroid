...
import com.example.wanAndroid.ui.adapter.CollectArticleAdapter
...
class CollectArticleFragment : Fragment() {
    ...
    /** 适配器 */
    private val adapter: CollectArticleAdapter by lazy { CollectArticleAdapter() }

    /** 数据集 */
    private lateinit var data: ApiResponse<ApiPagerResponse<ArrayList<CollectArticleResponse>>>
    ...
    private fun onRefresh() {
        page.onRefresh {
            scope {
                //获取收藏文章列表数据
                data = Get<ApiResponse<ApiPagerResponse<ArrayList<CollectArticleResponse>>>>("${NetApi.CollectArticleListAPI}/$index/json").await()
